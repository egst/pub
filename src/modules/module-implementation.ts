...
export class ModuleImplementationWrapper {
    ...
    init (errorHandler: (error: Error) => void): void {
        ...
        if (result instanceof util.FailedAttempt)
            errorHandler(result.error)
    }

    run (errorHandler: (error: Error) => void): void {
        const result = util.attempt(() =>
            (this.implementation as any).run()
        )
        if (result instanceof util.FailedAttempt) {
            errorHandler(result.error)
            return
...
export class ModuleImplementationConstructorWrapper {
    ...
    static createFromResponse (response: ValidResponse): util.Attempt<ModuleImplementationConstructorWrapper> {
        return util.attempt(() => {
            return new ModuleImplementationConstructorWrapper(eval(`(${response.code})`))
        })
    }

    construct (gui: HTMLElement): util.Attempt<ModuleImplementationWrapper> {
        return util.attempt(() => {
